...
ticket.prototype.assignee_edit_obj = function(assignees) {
	...
	assignees.each(function( assignee) {
		var selected = ( assignee.get('name') === this.assignees.data[0] ) ? 'selected="selected"' : '';
		select.append('<option ' + selected + '>' + assignee.get('name') + '</option>');
	});
